...
{
    public partial class UcHospitalInfoInput : UserControl
    {
		...
		{
			int serial = hospitalRepo.AutoIDValue();
			hospitalRepo.HospitalId= (++serial).ToString("d4");
			this.tbId.Text = hospitalRepo.HospitalId;
		}
	...
	*/
	...
	/*	private bool NullField()
		{
			if (this.tbPhone.Text == "" && this.tbArea.Text == "" && this.tbHouse.Text == "" && this.tbRoad.Text == "")
			{
				return false;
			}
			else
		...
		private void UcHospitalInfoInput_Load(object sender, EventArgs e)
		{
			//this.LoadAutoId();
			this.ComboBoxView();
